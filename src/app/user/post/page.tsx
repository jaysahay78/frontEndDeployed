...
import { createPost as doCreatePost, uploadPostImage } from "@/utils/post-service";
import { toast } from "@/hooks/use-toast";
import { User } from "@/utils/user-service";
import { getCurrentUserDetail, getToken } from "@/app/(auth)";
import {
  HoverCard,
  HoverCardContent,
  HoverCardTrigger,
...
export default function NewPost() {
  ...
  const userData = getCurrentUserDetail();
      ...
      useEffect(
          ()=>{
            if (userData) {
              setUser(userData);
              setPost(prev => ({ ...prev, userId: userData.id }));
            }
      ...
      const fieldChanged = (
        ...
        if (typeof eventOrName === "string" && value !== undefined) {
          // Manual call (e.g. from Select)
          setPost({ ...post, [eventOrName]: value });
        } else {
          // Event-based call (e.g. from Input)
    ...
    const createPost = (event: React.FormEvent<HTMLFormElement>)=>{
      ...
      doCreatePost(post).then(data=>{
        ...
        if (selectedImage) {
          uploadPostImage(selectedImage, data.postId)
            .then(() => {
              toast({
                variant: "success",
  ...
  return (
    ...
    <div className="container mx-auto py-0 px-4 md:px-6 flex flex-col min-h-screen">
        ...
        <h1 className="text-3xl font-bold mb-8 mt-2 flex">Create New Post</h1>
        <form className="max-w-3xl space-y-6" onSubmit={createPost}>
          <div className="pb-1">
          <Select
              value={post.categoryId !== null ? String(post.categoryId) : ""}
              onValueChange={(val) => fieldChanged("categoryId", val)}
              required
            >
              <SelectTrigger className="w-[280px] bg-slate-200">
                <SelectValue placeholder="Select a category"/>
              </SelectTrigger>
              <SelectContent>
                {
                categories.map((category)=>(
                    <SelectItem key={category.categoryId} value={category.categoryId.toString()}>{category.categoryTitle}</SelectItem>
                ))
                }
              </SelectContent>
            </Select>
